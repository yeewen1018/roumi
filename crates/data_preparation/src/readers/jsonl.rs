...
impl JsonlSource {
    ...
    pub fn stream<T: DeserializeOwned>(&self) -> Result<Box<dyn Iterator<Item = Result<T>> + Send>> {
        self.stream_impl()
    }
    ...
    fn stream_impl<T: DeserializeOwned>(&self) -> Result<Box<dyn Iterator<Item = Result<T>> + Send>> {
